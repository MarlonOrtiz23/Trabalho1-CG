// COMMAND LINE = Input_File_Name=biscuit  Initial_Frame=1 Final_Frame=180 Output_File_Name='Local+NomeArquivoSaída'
...
#if (clock < 0.5)           //acrescentado if (se os frames estão abaixo da metade..)
        #declare var1 = clock;    
#else
        #declare var1 = 0.5;  // permanecer a tela parada na metade dos frames em diante
#end
camera {location <20,40,28> direction var1*z look_at <0,2,0>}  // direção editada (original = 2*z)
...
#declare var = 2;                        //acrescentado
#if (clock >= 0.5)
        #declare var = (clock*10)-3;    
#end
object {Biscuit_2 rotate y*-80 translate <-3.5,0,var>}    //editar z da translação do objeto
...
object {TeaCup rotate y*50 translate <var+2,0,-14>}  // x editada (original = 4)
